...
// 绑定学期全选checkbox事件
function bindSemCheckboxEvents() {
    // 先解绑所有相关事件，防止重复绑定
    $('.x-sem-checkbox').off('change.sem').off('input.sem').on('change.sem', function () {
        const $semRow = $(this).closest('.x-sem-row');
        const year = $semRow.data('year');
        const sem = $semRow.data('sem');
        // 选中/取消本学期所有课程
        $('table:eq(1) tr:gt(0)').each(function () {
            if (
                $(this).find(`td:eq(${COL_INDEX.COURSE_YEAR})`).text() === String(year) &&
                parseInt($(this).find(`td:eq(${COL_INDEX.COURSE_SEMESTER})`).text()) === parseInt(sem)
            ) {
                $(this).find(`td:eq(${COL_INDEX.COURSE_CODE}) input[name="x-course-select"]`).prop('checked', $('.x-sem-checkbox#x-sem-check-' + year + '-' + sem).prop('checked'));
            }
        });
        // 联动课程类别checkbox
        ...
        // 联动所有学期checkbox
        $('.x-sem-checkbox').each(function () {
            const year = $(this).closest('.x-sem-row').data('year');
            const sem = $(this).closest('.x-sem-row').data('sem');
            let allChecked = true;
            let hasCourse = false;
            $('table:eq(1) tr:gt(0)').each(function () {
                if (
    ...
    // 课程checkbox联动学期checkbox
    $('input[name="x-course-select"]').off('change.sem').on('change.sem', function () {
        // 对每个学期checkbox，判断本学期所有课程是否全选
        $('.x-sem-row').each(function () {
            ...
            // 只有有课程时才设置checkbox
            if (hasCourse) {
                $(this).find('.x-sem-checkbox').prop('checked', allChecked);
            }
        });
    });
}
    ...
    $('table:eq(1)')
        ...
        .each(function () {
            ...
            if (time[0] !== year || time[1] !== sem) {
                ...
                $(this).before(`
              <tr class="x-sem-row" data-year="${year}" data-sem="${sem}">
                  ...
                  <span class="x-check-wrapper">
                    ...
                    <input type="checkbox" class="x-sem-checkbox" id="${semId}" checked />
                  </span>
                  </strong>
                  </td>
              </tr>
...
function bindEvents() {
    ...
    $('#x-sel-all').click(() => {
        if ($('input[name="x-course-select"]:checked').length === 0) {
            ...
            // 联动学期checkbox
            $('.x-sem-checkbox').prop('checked', false);
            $('#x-sel-all').text('全选');
        }
        updateAllScores();
    });
